@extends('layouts.visible')

@section('content')
    @include('visible.particles.breadcrumbs')

    <div class="contacts">
        <div class="wrapper wrapper_big">
            <div class="contacts__body flex">
                <div class="contacts__left">
                    <div class="contacts__title title bottom">{{ $page->name }}</div>
                    <div class="contacts__subtitle bottom">{!! $page->data['text'] ?? '' !!}</div>
                    <div class="contacts__social bottom">
                        <div class="contacts__social-items">
                            @isset($page->data['telegram'])
                                <div class="contacts__social-item">
                                <a href="https://t.me/{{ $page->data['telegram'] }}" class="contacts__link flex" target="_blank">
                                    <div class="contacts__link-icon flex-center">
                                        <svg width="15" height="11" viewBox="0 0 15 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M0.213966 5.12217C1.66326 4.38508 3.28106 3.76986 4.79265 3.15152C7.39317 2.13872 10.004 1.14348 12.6412 0.216917C13.1543 0.0590495 14.0763 -0.0953169 14.1667 0.606754C14.1171 1.60057 13.9135 2.58856 13.7739 3.57656C13.4194 5.74925 13.0096 7.9145 12.61 10.08C12.4723 10.8014 11.4937 11.1748 10.8675 10.7132C9.36264 9.77462 7.8462 8.84516 6.36054 7.88483C5.87387 7.42824 6.32516 6.77253 6.75979 6.44648C7.99926 5.31863 9.31372 4.3604 10.4884 3.17427C10.8053 2.46778 9.86903 3.06319 9.56021 3.24564C7.86338 4.32531 6.20808 5.4709 4.41909 6.41978C3.50527 6.88424 2.4402 6.48733 1.5268 6.22816C0.707839 5.91507 -0.49227 5.59963 0.213883 5.12226L0.213966 5.12217Z"
                                                fill="#20C4F3" />
                                        </svg>
                                    </div>
                                    <div class="contacts__link-caption">{{ '@'.$page->data['telegram'] }}</div>
                                </a>
                            </div>
                            @endisset

                            @isset($page->data['email'])
                                <div class="contacts__social-item">
                                    <a href="mailto:{{ $page->data['email'] }}" class="contacts__link flex">
                                        <div class="contacts__link-icon flex-center">
                                            <svg width="14" height="15" viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M6.99998 0.5C3.14016 0.5 0 3.64017 0 7.49996C0 11.3598 3.14016 14.5 6.99998 14.5C8.41408 14.5 9.77756 14.079 10.9429 13.2825L10.9632 13.2686L10.0201 12.1723L10.0041 12.1826C9.1069 12.7598 8.06809 13.0648 6.99998 13.0648C3.93151 13.0648 1.43517 10.5684 1.43517 7.49996C1.43517 4.43148 3.93151 1.93513 6.99998 1.93513C10.0684 1.93513 12.5648 4.43148 12.5648 7.49996C12.5648 7.89759 12.5207 8.30018 12.4338 8.69623C12.2577 9.4184 11.7522 9.63966 11.3723 9.61035C10.9905 9.57955 10.5436 9.30731 10.5407 8.64134V7.49996C10.5407 5.5476 8.95233 3.95926 6.99998 3.95926C5.04763 3.95926 3.45925 5.5476 3.45925 7.49996C3.45925 9.45236 5.04763 11.0407 6.99998 11.0407C7.94827 11.0407 8.83795 10.6698 9.50852 9.99552C9.89839 10.6023 10.5342 10.9828 11.2573 11.0409C11.3194 11.0459 11.3827 11.0484 11.4457 11.0484C11.9549 11.0484 12.4594 10.8782 12.8664 10.5693C13.286 10.2507 13.5993 9.78988 13.7722 9.23659C13.8002 9.14721 13.8508 8.94241 13.851 8.94109L13.8525 8.93372C13.9545 8.48941 14 8.04676 14 7.49996C14 3.64017 10.8598 0.5 6.99998 0.5Z"
                                                    fill="#20C4F3" />
                                                <path class="white" d="M9 7.49996C9 8.60279 8.10279 9.5 7 9.5C5.89717 9.5 5 8.60279 5 7.49996C5 6.39717 5.89717 5.5 7 5.5C8.10279 5.5 9 6.39717 9 7.49996Z" fill="white" />
                                            </svg>
                                        </div>
                                        <div class="contacts__link-caption">{{ $page->data['email'] }}</div>
                                    </a>
                                </div>
                            @endisset
                        </div>
                    </div>
                </div>
                <div class="contacts__right">
                    <form action="#" class="contacts__form form">
                        <label class="form__label bottom ">
                            <input type="text" name="text" class="form__field field validate--js" placeholder="Your name">
                        </label>
                        <label class="form__label bottom ">
                            <input type="text" name="text" class="form__field field validate--js validate-email--js" placeholder="E-Mail">
                        </label>
                        <label class="form__label bottom">
                            <input type="text" name="text" class="form__field field validate--js" placeholder="Your telegram">
                        </label>
                        <label class="form__label bottom ">
                            <textarea class="form__field field validate--js" placeholder="Message"></textarea>
                        </label>
                        <div class="form__box bottom">
                            <div class="form__caption">TYPE OF INQUIRY</div>
                            <div class="form__checkboxs flex">
                                @foreach($message_subjects as $subject)
                                    <label class="form__checkbox">
                                        <input type="radio" value="checkbox" name="subject" {{ $loop->first ? 'checked' : '' }}>
                                        <div class="form__checkbox-icon flex-center">
                                            <img src="/img/contacts/check.svg" alt="">
                                        </div>
                                        <div class="form__checkbox-caption">@lang("template.message.subject.$subject")</div>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                        <div class="form__captcha bottom">
                            This site is protected by reCAPTCHA and the Google
                            <a href="https://policies.google.com/privacy">Privacy Policy</a> and
                            <a href="https://policies.google.com/terms">Terms of Service</a> apply.
                        </div>
                        <button class="form__btn btn btn_blue bottom">
                            <div class="btn__caption">send message</div>
                            <div class="btn__icon svg-contain">
                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd"
                                          d="M2.47144 7.45148C2.2052 7.48813 2 7.71973 2 7.99997C2 8.30569 2.24421 8.55352 2.54545 8.55352L12.1333 8.55352L8.66986 12.0542L8.61692 12.1162C8.45804 12.3326 8.47501 12.6401 8.66826 12.8371C8.88083 13.0537 9.22619 13.0544 9.43965 12.8387L13.8323 8.39931C13.8543 8.37789 13.8745 8.35463 13.8927 8.32976L13.8927 8.32969C14.052 8.11265 14.0343 7.80406 13.8396 7.60737L9.43962 3.16129L9.37832 3.10782C9.16445 2.9475 8.86146 2.96603 8.66823 3.16297C8.45567 3.37961 8.45642 3.7301 8.66989 3.9458L12.1343 7.44642L2.54545 7.44642L2.47144 7.45148Z"
                                          fill="white" />
                                </svg>
                            </div>
                        </button>
                    </form>
                </div>
            </div>
        </div>
    </div>
@endsection
